...
def get_facerender_data(coeff_path, pic_path, first_coeff_path, audio_path, 
    ...
    if remainder!=0:
        target_semantics_list.extend(
            target_semantics for _ in range(batch_size - remainder)
        )
...
def transform_semantic_1(semantic, semantic_radius):
    semantic_list = [semantic for _ in range(0, semantic_radius*2+1)]
...
def gen_camera_pose(camera_degree_list, frame_num, batch_size):
    ...
    if len(camera_degree_list) == 1:
        new_degree_list.extend(camera_degree_list[0] for _ in range(frame_num))
        remainder = frame_num%batch_size
        if remainder!=0:
            new_degree_list.extend(
                new_degree_list[-1] for _ in range(batch_size - remainder)
            )
        return np.array(new_degree_list).reshape(batch_size, -1)
    degree_sum = 0.
